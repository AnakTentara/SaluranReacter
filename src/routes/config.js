...
export default function configRouter(botManager, io) {
  ...
  router.post('/accounts', async (req, res) => {
    try {
      const { id, name, reactProbability, minDelaySeconds, maxDelaySeconds } = req.body;
      if (!id) return res.status(400).json({ ok: false, error: 'id is required' });
      if (!name) return res.status(400).json({ ok: false, error: 'name is required' });

      const account = addAccount({ id, name, reactProbability, minDelaySeconds, maxDelaySeconds });
